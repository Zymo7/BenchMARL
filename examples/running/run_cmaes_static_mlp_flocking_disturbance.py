"""Dynamic-adaptation evaluation for trained static-MLP on flocking.

Loads a trained CMA-ES+static-MLP model and runs disturbance episodes
where at step ``--disturbance-step`` a disturbance is applied to agent
``--frozen-agent-idx``. Two disturbance modes are supported
(``--disturbance-mode``):

  - ``freeze`` (default): the action of the chosen agent is
    overridden to zero from the disturbance step onward and its
    position/velocity/force are pinned each step (it "freezes" in
    place, behaving like a static obstacle). The remaining agents
    continue to run the static-MLP policy.
  - ``push``: a one-shot velocity impulse (default direction =
    radial-out, magnitude = ``--push-magnitude``) is applied to the
    chosen agent at the disturbance step. No action zeroing, no
    anchoring — the agent must recover on its own.

Goal: compare against HAN's disturbance robustness — static-MLP has
no Hebbian plasticity so a frozen agent permanently removes degrees
of freedom; HAN's plastic weights may re-route around the frozen or
pushed agent.
...
Usage:
    /home/zhaozeming/miniconda3/envs/benchmarl/bin/python \\
        ...
        --experiment-path outputs/<your static-mlp experiment> \\
        --fitness-mode flocking_orbit \\
        --disturbance-mode freeze \\
...
from benchmarl.algorithms.cmaes_static_mlp import CmaesStaticMlpConfig
from benchmarl.algorithms.cmaes_static_mlp_optimizer import CmaesStaticMlpOptimizer
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Dynamic-adaptation evaluation: freeze one agent "
                    "mid-episode to test static-MLP robustness."
    )
    p.add_argument("--experiment-path", type=str, required=True,
                   help="Trained experiment folder (contains static_mlp_results/).")
    ...
    p.add_argument("--hidden-size", type=int, default=40,
                   help="Static-MLP hidden layer size. Default 40 matches "
                        "the 560-param static-MLP "
                        "(10*40 + 40*4 = 560 weights).")
...
def _load_trained_model(experiment, exp_path, device):
    """Load policy_state.pt + weights from a trained static-MLP experiment."""
    slm_dir = Path(exp_path) / "static_mlp_results"
    metadata_path = slm_dir / "results.json"
    with open(metadata_path, "r") as f:
        metadata = json.load(f)
    print(f"Loaded metadata: total_weights={metadata.get('total_weights')}, "
          f"fitness={metadata.get('best_fitness'):.4f}, "
    ...
    if weights_path.exists():
        ...
        print(f"Loaded weights: {len(weights)} parameters")
    return static_mlp_model, metadata


def _build_optimizer(experiment, static_mlp_model, device):
    """Build a CmaesStaticMlpOptimizer just so we can reuse its fitness
    component math (it has no _run_one_episode called in this script)."""
    return CmaesStaticMlpOptimizer(
        experiment=experiment,
...
def _compute_per_step_metrics(optimizer, pos_history, rot_history,
    ...
    for t in range(T):
        ...
        adj = (dist < nr) & (~eye)
        num_groups = CmaesStaticMlpOptimizer._count_connected_components(
            adj)
        ...
        # Scheme E weights (matches the optimizer's fitness exactly):
        # F_orbit = 1.5·At + 1.0·Dt + 0.2·Cg + 0.8·S
...
def run_disturbance_episode(optimizer, env, group, max_steps, policy,
    ...
    while not done and step < max_steps:
        ...
        # freeze override targets env 0 / agent `frozen_idx`. Slicing
        # [:, idx] freezes that agent in ALL envs (so pos_history etc.
        # show the same per-env behavior we expect).
...
def print_phase_summary(metrics, pos_history, target_pos_history,
    ...
    if not math.isnan(stat(Fg, pre)[0]):
        ...
        if drop < 0.1:
            print("  → static-MLP is ROBUST to the disturbance "
                  "(fitness barely changed).")
        elif drop < 0.4:
            print("  → static-MLP shows partial adaptation "
                  "(some drop, partial recovery).")
        else:
            print("  → static-MLP does NOT adapt well to the disturbance.")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    if args.disturbance_mode == "freeze":
        print("Dynamic-Adaptation Eval — Flocking with Frozen Agent")
    else:
        print("Dynamic-Adaptation Eval — Flocking with Push Disturbance")
    print("=" * 70)
    ...
    static_mlp_model, metadata = _load_trained_model(experiment, exp_path, device)
    optimizer = _build_optimizer(experiment, static_mlp_model, device)
    ...
    for ep in range(args.num_episodes):
        ...
        # Static-MLP has no Hebbian state to reset between episodes —
        # the weights are the fixed CMA-ES optimum. Skip the reset step.
