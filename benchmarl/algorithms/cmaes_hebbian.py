...
class CmaesHebbian(Algorithm):
    """CMA-ES trained Full Hebbian network.

    Single-phase training: CMA-ES directly optimizes all ABCD Hebbian parameters.
    No gradient-based (PPO) phase. The policy is a deterministic actor wrapped
    in a ProbabilisticActor for compatibility with the experiment framework.
    """
    ...
    def _get_loss(
        ...
        # a dummy loss for the framework. We use a simple MSE loss on zeros.
    ...
    def _get_parameters(self, group: str, loss) -> Dict[str, Iterable]:
        # All parameters are trainable (needed for framework compatibility)
        # but actual training is done by CMA-ES on ABCD buffers
        return {
    ...
    def get_full_hebbian_model(self, group: str = None):
        """Navigate the policy to find the FullHebbianModel instance."""
        if group is None:
        ...
        from benchmarl.models.full_hebbian import FullHebbianModel
        for sub_module in policy.module.modules():
            if isinstance(sub_module, FullHebbianModel):
                return sub_module
...
@dataclass
class CmaesHebbianConfig(AlgorithmConfig):
    """Config for :class:`~benchmarl.algorithms.CmaesHebbian`."""
