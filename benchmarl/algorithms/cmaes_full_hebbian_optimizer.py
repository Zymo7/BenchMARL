...
class CmaesFullHebbianOptimizer:
    """CMA-ES optimizer for ABCD parameters across ALL Hebbian layers.

    Single-phase training: CMA-ES directly optimizes the ABCD Hebbian parameters
    for all layers in the FullHebbian network.

    Supports multiple fitness modes:
      - "navigation": Reward based on reaching the goal position
      - "navigation_avoidance": Reward for navigation + obstacle avoidance penalty
    """
    ...
    def __init__(
        ...
        self.full_hebbian_model = full_hebbian_model
        ...
        self._initial_abcd = full_hebbian_model.get_abcd_vector().clone()
        self._best_abcd_so_far = None
        self._best_fitness_so_far = float('inf')
        self._current_gen = 0

    def get_abcd_vector(self) -> np.ndarray:
        return self.full_hebbian_model.get_abcd_vector().detach().cpu().numpy()

    def set_abcd_from_vector(self, x: np.ndarray):
        self.full_hebbian_model.set_abcd_from_vector(torch.tensor(x, device=self.device))
    ...
    def _compute_fitness(self, episode_reward: float, collision_count: int, total_steps: int,
        ...
        if self.fitness_mode == "navigation":
            ...
            # Progress term: how much of the initial distance was covered,
            # averaged over agents, normalized to [0, 1] by the initial distance.
            # progress=1 means an agent reached its goal; 0 means it did not move.
            ...
            # Mean progress across agents that actually had a non-zero start
            # (guards against degenerate episodes where agent==goal at reset).
            ...
            # Sparse bonus when all agents reach their goals.
            success_term = 1.0 if success else 0.0

            # Final-distance shaping (small weight): pulls the search toward
            # getting close even when progress is already plateaued.
            mean_final = final_goal_dists.mean().item() if mask.any() else 0.0
            final_term = -mean_final

            # Weights chosen so progress dominates (~1.0), success gives a
            # strong but bounded bonus (~5.0), final-term is a small pull
            # (~0.1 per unit distance). All are on comparable scales.
            w_progress = 1.0
            w_success = 5.0
            w_final = 0.1
    ...
    def _run_one_episode(self, env, group, max_steps, policy, on_frame=None):
        ...
        Args:
            env: the test environment (already set up with a seed).
            group: the agent group key.
            max_steps: hard cap on steps.
            policy: the policy module.
            on_frame: optional callable ``on_frame(td)`` invoked after each
                step (used by ``evaluate`` to capture video frames).

        Returns:
            dict with keys: episode_reward, collision_count, step, success,
            initial_goal_dists, final_goal_dists.
        """
        td = env.reset()

        # Initial per-agent distance to goal: the first 2 obs entries are
        # (agent.pos - goal.pos), so ||obs[:2]|| is the distance.
        ...
        # Final per-agent distance to goal (after the last step).
        # Whether we terminated or were truncated, the current `td` holds the
    ...
    def fitness(self, x: np.ndarray) -> float:
        ...
        self.full_hebbian_model.reset_all_weights()
    ...
    def run(self) -> np.ndarray:
        ...
        # Per-layer breakdown
        layers = self.full_hebbian_model.get_all_hebbian_layers()
        layer_info = ", ".join(
    ...
    def plot_convergence(self, output_dir: str):
        ...
        ax1.set_title(f"CMA-ES Convergence (Full Hebbian, mode={self.fitness_mode})")
        ax1.legend()
    ...
    def save(self, output_dir: str):
        ...
        # Save individual layer ABCD params
        layers = self.full_hebbian_model.get_all_hebbian_layers()
        for i, layer in enumerate(layers):
            np.save(os.path.join(abcd_dir, f"layer{i}_abcd.npy"), layer.get_abcd_vector().detach().cpu().numpy())

        torch.save(self.policy.state_dict(), os.path.join(abcd_dir, "policy_state.pt"))

        metadata = {
            "fitness_mode": self.fitness_mode,
            "total_abcd_params": self.full_hebbian_model.total_abcd_params,
            "n_layers": len(layers),
            "layer_shapes": [
        ...
        print(f"  Full Hebbian results saved to: {abcd_dir}/")
        return abcd_dir
    ...
    def evaluate(
        ...
        Args:
            output_dir: directory to write ``videos_hebbian/`` into.
            n_episodes: number of evaluation episodes.
            fps: frames per second written to the mp4. Larger fps → shorter video.
            max_video_frames: if set, stop recording after this many frames per
                episode (the env still runs to ``done`` or ``max_steps``; only
                frame capture is truncated). Useful when each episode is long
                and the resulting videos are too big to inspect.
        """
        import torchvision
        ...
        with torch.no_grad(), set_exploration_type(ExplorationType.DETERMINISTIC):
            for ep in range(n_episodes):
                ...
                # Per-episode frame-capture closure. Records one frame per call
                # until `max_video_frames` is hit (if set), then stops rendering
                # so the env keeps running without the render overhead.
                ...
                # Reset-frame capture (one frame before any step).
                try:
                    frame = env.render(mode="rgb_array")
                    if frame is not None:
